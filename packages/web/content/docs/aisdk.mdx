...
set the `MODEL` env var to swap the model at runtime without changing code. useful for testing new models against existing chaos configurations

```bash
MODEL=gpt-5 bun run examples/ai-sdk/generate-text/openai/basic.ts
```

for gateway models, the provider prefix is preserved:
