...
const network = {
	latency: <T extends AnyFn>(fn: T, ms: number | [number, number]): T => cruel(fn, { delay: ms }),
	...
	slow: <T extends AnyFn>(fn: T): T => cruel(fn, { delay: [1000, 5000], jitter: 2000 }),
	...
	offline: <T extends AnyFn>(_fn: T): T => {
...
const ai = {
	...
	hallucinate: <T extends AnyFn>(fn: T): T => cruel(fn, { delay: [100, 500] }),

	thinking: <T extends AnyFn>(fn: T): T => cruel(fn, { delay: [2000, 10000] }),
...
cruel.scope = async <T>(fn: () => MaybePromise<T>, opts: ChaosOptions = {}): Promise<T> => {
...
cruel.patchFetch = (): void => {
	...
	globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
		...
		for (const rule of state.intercepts) {
			const matches =
				typeof rule.pattern === "string" ? url.includes(rule.pattern) : rule.pattern.test(url)
...
const scenarios: Record<string, Omit<ScenarioConfig, "name">> = {
	...
	blackFriday: {
		chaos: { delay: [1000, 3000] as [number, number], fail: 0.15, jitter: 1000 },
		duration: 60000,
	},
	mobileNetwork: {
		chaos: { delay: [500, 2000] as [number, number], fail: 0.2, timeout: 0.1 },
		duration: 30000,
	},
	datacenterFailover: {
		chaos: { fail: 0.5, delay: [200, 800] as [number, number] },
		duration: 20000,
	},
	ddosAttack: {
		chaos: { timeout: 0.4, delay: [2000, 10000] as [number, number] },
		duration: 30000,
	},
	coldStart: { chaos: { delay: [3000, 8000] as [number, number] }, duration: 10000 },
	gcPause: {
		chaos: { delay: [100, 500] as [number, number], spike: [500, 2000] as [number, number] },
		duration: 15000,
	},
...
function emitEvent(
	type: CruelEventType,
	data: { target?: string; error?: Error; duration?: number } = {},
) {
...
function createCruel(defaults: ChaosOptions = {}) {
	...
	instance.flaky = <T extends AnyFn>(fn: T, intensity?: number) => cruel.flaky(fn, intensity ?? 0.2)
...
export {
	...
	type CruelMiddlewareOptions,
	type CruelModelOptions,
	type CruelProviderOptions,
	contentFilterError,
	contextLengthError,
	cruelMiddleware,
	cruelModel,
