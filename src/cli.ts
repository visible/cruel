...
async function testEndpoint(url: string, opts: Record<string, string | number | boolean>) {
  ...
  if (retryAttempts) console.log(colors.dim(`retry: ${retryAttempts} attempts`))
  if (circuitThreshold) console.log(colors.dim(`circuit breaker: ${circuitThreshold} threshold`))
  console.log()

  let fetchFn: typeof fetch = cruel(fetch, chaosOpts)
...
async function benchmark(url: string, opts: Record<string, string | number | boolean>) {
  ...
  switch (command) {
    ...
    case "scenario":
      if (!opts._target) {
        ...
        console.log(colors.dim("available: networkPartition, highLatency, degraded, outage, recovery"))
        return
      }
